...
namespace Winform_Bai_Toan_Thap_Ha_Noi
{
    public partial class Form1 : Form
    {
        ...
        // Tạo lớp Stack để quản lý các đĩa trên mỗi cột trong bài toàn Tháp Hà Nội, với các phương thức để thêm, lấy và xem đĩa trên cột, cũng như lấy tất cả đĩa hiện có và xóa tất cả đĩa khi cần thiết
        class Stack
        {
            private List<int> data = new List<int>();

            public void Push(int x)// Thêm một đĩa có kích thước x vào đỉnh của ngăn xếp, kiểm tra để đảm bảo rằng luật của Tháp Hà Nội không bị vi phạm (đĩa lớn hơn không được đặt trên đĩa nhỏ hơn)
            {
                if (data.Count > 0 && data[data.Count - 1] < x)
                    throw new Exception("Sai luật Hanoi rồi bạn ơi ");

                data.Add(x);
            }

            public int Pop()//lấy đĩa ở đỉnh của ngăn xếp và loại bỏ nó khỏi ngăn xếp, trả về kích thước của đĩa đã lấy; nếu ngăn xếp rỗng, trả về 0
            {
                if (data.Count == 0) return 0;
                int val = data[data.Count - 1];
                data.RemoveAt(data.Count - 1);
                return val;
            }

            public int Peek()// trả về đĩa ở đỉnh mà không loại bỏ nó
            {
                if (data.Count == 0) return 0;
                return data[data.Count - 1];
            }

            public List<int> GetAll() => data;
        ...
        Stack Get(char c)// phương thức để lấy ngăn xếp tương ứng với cột được chỉ định bởi ký tự c ('A', 'B' hoặc 'C'), giúp đơn giản hóa việc truy cập và thao tác với các ngăn xếp đại diện cho ba cột trong trò chơi Tháp Hà Nội
        {
            if (c == 'A') return A;
            if (c == 'B') return B;
            return C;
